...
with wave.open("./CMU_ARCTIC/cmu_us_aew_arctic/wav/arctic_a0001.wav") as wav_speech:
    ...
    room.add_source(source_locs[0], signal=speech_data)
    ...
    vector_near = calculate_steering_vector(mic_locs, source_locs.T, frequencies=f, use_far=False)
    ...
    wave_writer.write("sample_simulated_noise_reduced.wav", ds_out, f_rate=frame_rate)

with wave.open("sample_simulated_with_noise.wav") as wav:
    print("playing simulated speech data, with noise")
    wave_loader.play(wav)

with wave.open("sample_simulated_noise_reduced.wav") as wav:
